 ...
 *
 * Hardware-layer code (DPMI, PCI, VBE, MMIO, GPU engine) lives in
 * RADEONHW.C / RADEONHW.H.  This file contains only the test-specific
 * code: output helpers, GPU 2D primitives, validation tests and main().
 */
...
/* RBLIT-specific: actual mapped LFB size (may exceed g_yres pages) */
static unsigned long g_lfb_size = 0;

/* Original VBE pitch before 64-byte GPU alignment.
   Kept for diagnostic display in test output and pitch-mismatch detection. */
static int g_vbe_pitch = 0;
